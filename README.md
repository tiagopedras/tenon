...
Eighteen hues by nineteen steps, plus four values that sit outside the ramps.
Every one of the 346 is generated by `scripts/generate-ramps.mjs` from a single
...
Six headings, `heading-1` through `heading-6`, plus stat, body, body-sm, label,
caption, caption-sm and code. `stat` is the figure in a Stat, and it is the one
style the build writes no class for, since the component owns `.tenon-stat`. They land at the small end of the ramp because the board is a
...
Text styles are shared by every theme, and a theme can replace some of them.
`tokens/semantic/typography.<theme>.json` holds only the styles that differ, and
the build writes them at the end of that theme's block, so every class and
component that reads a text style follows. Today only `tiagopedras_2026` has
...
refuses to write either if any of six checks fail: light, dark and tiagopedras_2026
must hold the identical key set, a theme's typography file may only replace text
styles the shared file has, every `{alias}` must resolve, no semantic token may
